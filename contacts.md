...
Unique custom fields participate in matching: when no `contacts_master_id` is supplied, VipeCloud matches on every attribute your account has marked unique — not only the standard fields, but any **custom field** configured as unique (account-wide or per user). If a submitted `custom_fields` value matches the value held by an existing contact on a unique custom field, that contact is **updated** rather than a second contact being created. This applies whether the `custom_fields` key is the custom field's ID or its name. Custom fields that are not marked unique never affect matching.

Unique rules also participate in matching. A unique rule is a combination of two or three fields that must be unique together, set up by an account admin under Setup → Customizations → Unique Rules — for example **Email + Brand**, which allows one contact per email address for each brand. When no `contacts_master_id` is supplied and the submitted values match an existing contact on every field in a rule, that contact is **updated** rather than a second contact being created. On a create, a field in the rule that is left out of the request or sent blank counts as blank, and matches a contact that has no value in that field. Custom fields in a rule are read from `custom_fields` under their ID or name, as above. When updating a contact by `contacts_master_id`, fields you don't send keep their stored values, and a change that would give the contact the same values as another contact under a rule is refused with an `exists_for_user` or `exists_in_account` status, the same as a duplicate unique field.
