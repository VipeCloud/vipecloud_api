...
template_id | integer | cond. | Required (and used) for template-backed actions ("email", "series", "text", "social", "task", "opportunity") on every item_type. **On item_type "contact_list" only**, the `template_id` key must additionally be present in the body — any value, `0` is conventional — for every action except "cancel_*", "custom_field" and "stop_ai_booking_agent", even actions that don't use the value at all ("ai_scheduling", "ai_brand_matching", "ai_contact_research", "add_to_list", "remove_from_list", "add_to_watchlist", "remove_from_watchlist"). See the footnote under [Actions](#actions-template_type) below. This presence-only requirement does NOT apply to item_type "contact", "opportunity", or "suite_party" — there, a missing template_id on a non-template-backed action is simply ignored.
template_ids | integer[] | cond. | Required on EVERY item_type when template_type is "cancel_email", "cancel_text", or "cancel_series" (or send `cancel_template_ids` instead — see below); a missing/absent key 422s (see [Errors](#errors)) regardless of item_type. Contains the template ids (of the matching type) whose scheduled sends/steps should be canceled when the automation fires.
cancel_template_ids | integer[] | no | Accepted as an alias for `template_ids`, on any item_type, when `template_ids` is not present in the same request AND `cancel_template_ids` is itself truthy (a present-but-empty array does not alias, and does not satisfy the `template_ids`-required check on any item_type — that combination still 422s). If both are present, `template_ids` wins.
...
422 | "Creating or updating a cancel_email/cancel_text/cancel_series automation requires a template_ids array (or cancel_template_ids)." | template_type is cancel_email/cancel_text/cancel_series and neither `template_ids` nor `cancel_template_ids` is present — checked on every item_type, before the permission check below, so this 422 wins even for a caller who also lacks permission
